...
public final class FieldHandler {
	 ...
	 *             If the parameters for the method are not valid (empty).
	 */
	public static Boolean isGetter(String methodName) throws InvalidParameterException {
	 ...
	 *             If the parameters for the method are not valid (empty).
	 */
	public static Boolean isSetter(String methodName) throws InvalidParameterException {
		// If the method name is null or empty.
		if (methodName == null) {
			// Throws an exception.
			throw new EmptyParameterException("class"); // TODO
	 ...
	 *             If the parameters for the method are not valid (empty).
	 */
	public static String getFieldName(String methodName) throws InvalidParameterException {
		// If the method name is null or empty.
		if (methodName == null) {
			// Throws an exception.
	...
	public static Object getFieldValueDirectly(Object object, final String fieldPath,
			final Boolean exceptionOnNull, final Boolean ignoreAccess) throws InvalidParameterException {
	 ...
	 */
	public static Object getFieldValue(Object object, final String fieldPath, final Boolean exceptionOnNull)
		...
		for (String currentFieldName : fieldNames) {
			...
			// Tries to get the next field value in the path.
			object = MethodHandler.invokeMethod(object, getGetterName(currentFieldName), null);
		}
		// Returns the field value.
		return object;
	}
	...
	public static void setFieldValueDirectly(Object object, final String fieldPath, final Object fieldValue,
		...
		for (Integer currentFieldIndex = 0; currentFieldIndex < fieldNames.length; currentFieldIndex++) {
			...
			// Gets the field for the current name.
			Field currentField = getField(object.getClass(), fieldNames[currentFieldIndex]);
			// Defines the accessibility of the field.
			currentField.setAccessible(ignoreAccess);
	...
	public static void setFieldValue(Object object, final String fieldPath, final Class<?> fieldClass,
			final Object fieldValue, final Boolean exceptionOnNull) throws InvalidParameterException,
			InvocationException {
		// If the passed object is null.
		if (object == null) {
			// Throws an exception.
		...
		for (Integer currentFieldIndex = 0; currentFieldIndex < fieldNames.length; currentFieldIndex++) {
			...
			// If it is the last field.
				...
				MethodHandler.invokeMethod(object, getSetterName(fieldNames[currentFieldIndex]),
						new Class<?>[] { fieldClass }, new Object[] { fieldValue });
			}
		}
	}
	...
	public static void setFieldValue(final Object object, final String fieldPath, final Object fieldValue,
		...
		Class<?>[] parametersClasses = MethodHandler.getParametersClasses(new Object[] { fieldValue });
		// Tries to set the field using the class.
		setFieldValue(object, fieldPath, parametersClasses[0], fieldValue, exceptionOnNull);
	}
}
