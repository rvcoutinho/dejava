package org.dejava.component.util.message.handler.impl;
...
import org.dejava.component.util.message.annotation.MessageBundle;
import org.dejava.component.util.message.annotation.MessageBundles;
...
public class DefaultMessageHandler implements I18nMessageHandler {
	 ...
	 */
	public DefaultMessageHandler(final Locale locale) {
		super();
		// Sets the locale.
		this.locale = locale;
	}
	 ...
	 */
	private String addsParameters(final String message, final Object[] parametersValues) {
	 ...
	 */
	private String getMessage(final String bundleBaseName, final String key, final Object[] parametersValues)
			throws MessageNotFoundException {
		...
		try {
			...
			return addsParameters(message, parametersValues);
		}
	...
	public String getMessage(final Locale locale, final String type, final String key,
			final Object[] parametersValues, final Boolean exceptionOnUnaccessible)
		...
		for (Integer currentStackDepth = 1; (currentClass != null)
			...
			try {
				...
				if (messageBundles != null) {
					...
					for (MessageBundle currentMessageBundle : messageBundles.messageBundles()) {
						...
						if (currentMessageBundle.type().equalsIgnoreCase(type)) {
							...
							return getMessage(currentMessageBundle.baseName(), key, parametersValues);
						}
					}
				}
	...
	/**
	 * @see org.dejava.component.util.message.handler.I18nMessageHandler#getMessage(java.util.Locale,
	 *      org.dejava.component.util.message.model.ApplicationMessageType, java.lang.String,
	...
	public String getMessage(final Locale locale, final ApplicationMessageType type, final String key,
			...
			throws MessageNotFoundException, InvalidParameterException {
		// Gets the message.
		return getMessage(locale, type.name(), key, parametersValues, exceptionOnUnaccessible);
	}
	...
	public String getMessage(final String type, final String key, final Object[] parametersValues,
			final Boolean exceptionOnUnaccessible) throws MessageNotFoundException, InvalidParameterException {
		// Gets the message.
		return getMessage(getLocale(), type, key, parametersValues, exceptionOnUnaccessible);
	}
	...
	public String getMessage(final ApplicationMessageType type, final String key,
			...
			throws MessageNotFoundException, InvalidParameterException {
		// Gets the message.
		return getMessage(type, key, parametersValues, exceptionOnUnaccessible);
	}
}
