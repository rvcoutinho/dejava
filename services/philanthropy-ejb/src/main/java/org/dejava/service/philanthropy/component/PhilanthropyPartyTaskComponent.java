 ...
 */
@PhilanthropyCtx
@Stateless(name = "Component/Philanthropy/PhilanthropyPartyTask")
public class PhilanthropyPartyTaskComponent {
	...
	@Inject
	@PartyCtx
	private PartyTaskComponent partyTaskComponent;
	 ...
	 */
	private void addPartyAdminPermissions(final PhilanthropyParty party) {
	 ...
	 */
	public PhilanthropyParty createPhilanthropyParty(final PhilanthropyParty party) {
		...
		// Persists the party.
		final PhilanthropyParty newParty = philanthropyPartyComponent.createParty(party);
		// Adds the administrator permissions for a party being created.
	 ...
	 */
	public Collection<PhilanthropyParty> getPartiesAdministeredByUser(final Integer userId,
			final Integer firstResult, final Integer maxResults) {
		// Creates the party list to be returned.
		final Collection<PhilanthropyParty> parties = new ArrayList<>();
	 ...
	 * @return The parties administered by a given user.
	 */
	public <PartyType extends PhilanthropyParty> Collection<PartyType> getPartiesAdministeredByUser(
			final Integer userId, final Class<PartyType> partyType, final Integer firstResult,
			final Integer maxResults) {
		// Gets the parties administered by the given user.
		final Collection<PhilanthropyParty> allParties = getPartiesAdministeredByUser(userId, firstResult,
				maxResults);
		// Creates the party list to be returned.
		final Collection<PartyType> partiesWithType = new ArrayList<>();
		// For each party administered by the given user.
		for (final PhilanthropyParty currentParty : allParties) {
